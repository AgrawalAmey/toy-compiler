...
int
createParseTree(parse_table pt, parseTree t, Token * input)
{
    // Initialize stack
    stackNode * s = NULL;
    ...
    while (isStackEmpty(&s) != 1) {
        ...
        if (tok >= 1000) {
            ...
            if (tok == input[readHead].name) {
                insertLeafDataInParseTree(input[readHead], t);
                readHead++;
            } else {
                ...
                printf(
                  ...
                  token_to_string(
                      input[readHead].name), input[readHead].string, input[readHead].line_number, token_to_symbol(tok));
                return -1;
            }
        } else {
            ...
            if (pt[tok - 500][input[readHead].name - 1000][0] != 0) {
                ...
                // Insert into tree
                insertInParseTree(pt[tok - 500][input[readHead].name - 1000], t);
            } else {
